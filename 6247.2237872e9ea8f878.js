...
class WaitPage {
  ...
  ngOnInit() {
    ...
    if (query) {
      ...
      let booleanValue = resultObject.is_whatsapp.toLowerCase() === "true" ? true : false;
      localStorage.setItem('isWPBooking', booleanValue);
      if (resultObject.origin_name) {
        localStorage.setItem('whatsappPrefilledData', JSON.stringify(resultObject));
      } else {
        localStorage.removeItem('whatsappPrefilledData');
      }
      if (resultObject.mobile.length > 10) {
        let splitNumber = {
          firstTwo: resultObject.mobile.slice(0, 2),
          remaining: resultObject.mobile.slice(2)
        };
        localStorage.setItem('wpMobileNo', splitNumber.remaining);
      } else {
        localStorage.setItem('wpMobileNo', resultObject.mobile);
      }
    }
